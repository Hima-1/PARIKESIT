...
@section('title', 'Dokumentasi Kegiatan ' . $dokumentasiKegiatan->judul_dokumentasi)

@section('content')
    @php
        $canManageDokumentasi = auth()->user()->role === 'admin' || $dokumentasiKegiatan->created_by_id === auth()->id();
    @endphp
    <div class="space-y-6">

        {{-- Judul --}}
        <div class="flex justify-between items-center">
            <h4 class="text-xl font-semibold text-gray-700">Dokumentasi Kegiatan</h4>
        </div>
        ...
        <nav class="bg-white p-4 border-2 rounded-md w-full mb-4">
            <ol class="list-reset flex text-grey-dark">
                ...
                <li><a href="{{ route('dokumentasi.index') }}" class="text-blue-600 hover:underline">Dokumentasi Kegiatan</a></li>
                <li><span class="mx-2">&gt;</span></li>
                <li class="text-gray-700">Dokumentasi Kegiatan : {{ $dokumentasiKegiatan->judul_dokumentasi }}</li>
            </ol>
        </nav>
            ...
            <div class="col-span-4 border border-gray-300 bg-white p-4 rounded-md shadow-sm">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-1 md:grid-cols-1 gap-4">
                    ...
                    <div class="flex items-center justify-between p-4 border rounded-md bg-white shadow-sm">
                        ...
                        <a href="{{ $dokumentasiKegiatan->publicFileUrl('bukti_dukung_undangan_dokumentasi') ?? '#' }}" target="_blank"
                    ...
                    <div class="flex items-center justify-between p-4 border rounded-md bg-white shadow-sm">
                        ...
                        <a href="{{ $dokumentasiKegiatan->publicFileUrl('daftar_hadir_dokumentasi') ?? '#' }}" target="_blank"
                            class="flex items-center text-gray-700 hover:text-black text-sm">
                            <i class="fas fa-eye mr-1"></i> Lihat
                        </a>
                    </div>
                    ...
                    <div class="flex items-center justify-between p-4 border rounded-md bg-white shadow-sm">
                        ...
                        <a href="{{ $dokumentasiKegiatan->publicFileUrl('notula_dokumentasi') ?? '#' }}" target="_blank"
                            class="flex items-center text-gray-700 hover:text-black text-sm">
                            <i class="fas fa-eye mr-1"></i> Lihat
                        </a>
                    </div>
                    ...
                    <div class="flex items-center justify-between p-4 bg-white rounded-md shadow-sm border border-gray-300">
                        ...
                        <a href="{{ $dokumentasiKegiatan->publicFileUrl('materi_dokumentasi') ?? '#' }}" target="_blank"
                            class="flex items-center text-gray-700 hover:text-black text-sm">
                            <i class="fas fa-eye mr-1"></i> Lihat
                        </a>
                    </div>
                        ...
                        @foreach ($dokumentasiKegiatan->file_dokumentasi as $index => $media)
                            <tr>
                                ...
                                <td class="px-6 py-4 border-b border-gray-300">
                                    <img src="{{ $media->publicUrl() ?? '#' }}" class="h-16 w-16 object-cover" alt="Media">
                                </td>
                                <td class="px-6 py-4 border-b border-gray-300">
                                    <a href="{{ $media->publicUrl() ?? '#' }}" target="_blank" class="bg-blue-500 hover:bg-blue-600 dark:bg-dark-400 dark:hover:bg-dark-500 transition duration-300 ease-in-out text-white hover:text-blue-50 font-semibold px-2 py-3 rounded mr-2">Lihat</a>
                                    @if ($canManageDokumentasi)
                                        <form action="{{ route('fileDok.destroy', $media->id) }}" method="POST" class="inline deleteMediaForm">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" class="deleteBtn bg-red-500 hover:bg-red-600 dark:bg-dark-400 dark:hover:bg-dark-500 transition duration-300 ease-in-out text-white hover:text-red-50 font-semibold px-2 py-3 rounded">Hapus</button>
                                        </form>
                                    @endif
                                </td>
                            </tr>
    ...
    <script>
        $('.deleteBtn').click(function(e) {
            ...
            Swal.fire({
                ...
                text: "Anda tidak dapat mengembalikan dokumentasi kegiatan ini!",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
                confirmButtonText: 'Ya, hapus media dokumentasi kegiatan ini!'
            }).then((result) => {
         ...
         $('#delete_parent').click(function(e) {
            ...
            Swal.fire({
                title: 'Apakah kamu yakin?',
                text: "Anda tidak dapat mengembalikan data dokumentasi kegiatan ini!",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
                confirmButtonText: 'Ya, hapus data dokumentasi kegiatan ini!'
            }).then((result) => {
                if (result.isConfirmed) {
                    e.preventDefault();
                    $('#form_delete').submit();
