...
void main() {
  ...
  group('Admin User Management', () {
    testWidgets('admin can create and search users', (tester) async {
      ...
      final addButton =
          (tester.view.physicalSize.width / tester.view.devicePixelRatio < 600)
          ? const Key('admin-user-add-mobile')
          : const Key('admin-user-add');

      await navigateTo(
        tester,
        find.text(userManagementText),
        targetText: 'Daftar Pengguna',
        label: 'User Management Navigation',
      );

      // 3. Create User
      const testUserName = '[TEST_ADMIN] User IT';
      const testUserEmail = 'test_it_user@example.com';
